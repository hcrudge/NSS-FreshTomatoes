...
export const MovieProvider = (props) => {
  ...
  const getMovies = () => {
    return fetch("http://localhost:8088/movies?_expand=friend")
      .then((res) => res.json())
  ...
  const getMovieByTMDBId = (TMDBId) => {
    return fetch(`http://localhost:8088/TMDB/${TMDBId}/movies`)
  ...
  const getMovieById = (id) => {
    return fetch(`http://localhost:8088/movies/${id}?_expand=friend`).then(
